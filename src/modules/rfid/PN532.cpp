...
int PN532::emulate_tag(String ndef_data)
{
    // Parameters for TgInitAsTarget, starting with the Mode byte.
    // This configures the PN532 to emulate a standard NFC Forum Type 4A Tag,
    // which is what modern phones, including iPhones, expect.
    uint8_t tgInitAsTargetParams[] = {
        0x04, // Mode: PICC only. Passive mode is implicit.
        ...
        0x34,
        0x56, // NFCID1
        0x20, // SEL_RES (0x20 indicates ISO/IEC14443-4 compliance)

        // FeliCa Params (18 bytes) and NFCID3t (10 bytes) - all zeros
        0x00,
        0x00,
        0x00,
        0x00,
    ...
    // The new library version splits TgInitAsTarget into two parts.
    if (!nfc.TgInitAsTarget() ||
        !nfc.inDataExchange(tgInitAsTargetParams, sizeof(tgInitAsTargetParams), response, &responseLength))
    {
        return FAILURE;
    }

    uint8_t compatibilityContainer[] = {
        0x00, 0x0F, 0x20, 0x00, 0x54, 0x00, 0xFF, 0x04, 0x06, 0xE1, 0x04, 0x00, 0xFF, 0x00, 0x00};

    TagFile currentFile = TagFile::NONE;

    check(AnyKeyPress); // Clear any pending key press before starting

    while (!check(AnyKeyPress))
    {
        uint8_t apdu[64];
        uint8_t apduLen = 64;
        uint8_t status = nfc.TgGetData(apdu, &apduLen);

        if (status != 0x00)
        {
            // Target has been released by the reader, or another error occurred.
            // We need to re-initialize as a target to accept new connections.
            if (!nfc.TgInitAsTarget() ||
                !nfc.inDataExchange(tgInitAsTargetParams, sizeof(tgInitAsTargetParams), response, &responseLength))
            {
                return FAILURE; // Could not re-initialize
            }
            currentFile = TagFile::NONE; // Reset state
            delay(50);
            continue;
        }
        ...
        if (ins == ApduCommand::ISO7816_SELECT_FILE)
        {
            if (p1 == ApduCommand::C_APDU_P1_SELECT_BY_ID)
            {
                ...
                else if (lc == 0x02 && apdu[5] == 0xE1 && (apdu[6] == 0x03 || apdu[6] == 0x04))
                {
                    currentFile = (apdu[6] == 0x03) ? TagFile::CC : TagFile::NDEF;
                    wbuf = {
                        ApduCommand::R_APDU_SW1_COMMAND_COMPLETE, ApduCommand::R_APDU_SW2_COMMAND_COMPLETE};
                }
                else
            ...
            {
                std::vector<uint8_t> application_name(apdu + 5, apdu + 5 + lc);
                if (std::equal(
        ...
        else if (ins == ApduCommand::ISO7816_READ_BINARY)
        {
            ...
            {
                if (p1p2Length > sizeof(compatibilityContainer))
                {
                    wbuf = {
                ...
                else
                {
                    ...
                    wbuf.insert(
                        wbuf.end(),
                        compatibilityContainer + p1p2Length,
                        compatibilityContainer + p1p2Length + lc);
            ...
            {
                // Construct the NDEF file in memory: 2-byte NLEN + NDEF message
                std::vector<uint8_t> ndef_file;
                std::vector<uint8_t> payload = Ndef::urlNdefAbbrv(std::string(ndef_data.c_str()));
                std::vector<uint8_t> ndef_message = Ndef::newMessage(payload);
                ndef_file.push_back((ndef_message.size() >> 8) & 0xFF);
                ndef_file.push_back(ndef_message.size() & 0xFF);
                ndef_file.insert(ndef_file.end(), ndef_message.begin(), ndef_message.end());

                if (p1p2Length >= ndef_file.size())
                {
                    wbuf = {
                        ApduCommand::R_APDU_SW1_END_OF_FILE_BEFORE_REACHED_LE_BYTES,
                        ApduCommand::R_APDU_SW2_END_OF_FILE_BEFORE_REACHED_LE_BYTES};
                ...
                {
                    uint8_t bytes_to_send = lc; // Le
                    if (p1p2Length + bytes_to_send > ndef_file.size())
                    {
                        bytes_to_send = ndef_file.size() - p1p2Length;
                    }
                    wbuf.insert(
                        wbuf.end(),
                        ndef_file.begin() + p1p2Length,
                        ndef_file.begin() + p1p2Length + bytes_to_send);
                    wbuf.push_back(ApduCommand::R_APDU_SW1_COMMAND_COMPLETE);
                    wbuf.push_back(ApduCommand::R_APDU_SW2_COMMAND_COMPLETE);
                }
            }
        ...
        if (!wbuf.empty())
        {
            nfc.TgSetData(wbuf.data(), wbuf.size());
        }
    }

    // Release the PN532 from target mode and reconfigure it as a reader.
    // This is crucial to ensure it can read tags again after emulation.
    // Calling SAMConfig is sufficient to exit target mode.
    if (!nfc.SAMConfig())
    {
        return FAILURE;
    }
...
int PN532::set_idle()
{
    // wakeup() now returns void, so we can't check its return value.
    nfc.wakeup();
    return SUCCESS;
}
...
int PN532::read_felica_data()
{
    ...
    for (uint16_t i = 0x8000; i < 0x8000 + totalPages; i++)
    {
        ...
        {
            if (res)
            { // If card block read successfully, copy data to string
                strPage = hexToStr(block_data[0], 16);
            }
        }
        if (res)
        { // If PN532 can't read the FeliCa tag, don't write the block to file
